...
use App\Http\Controllers\Modals\PurchaseOrderItems;
...
class PurchaseOrderItemController extends Controller
{
    public function index(Request $request, $purchaseOrderId)
    {
        if (request()->ajax()) {
            ...
            ->editColumn('item.name', function ($query) {
                if ($query->purchaseOrder->status->name == 'Draft') {
                    return '
                        <div class="tom-select-custom">
                            <select name="purchase_order_items[' . $query->id . '][item]" class="form-select" autocomplete="off">
                                <option
                                    selected=""
                                    value="' . $query->item->id . '"
                ...
                if ($query->purchaseOrder->status->name == 'Draft') {
                    return '
                        <div class="input-group input-group-merge">
                            ...
                            <input id="price" name="purchase_order_items[' . $query->id . '][quantity]" type="text" class="input-count form-control text-end" placeholder="" value="' . number_format($query->quantity, 2, '.', '') . '" autocomplete="off" style="min-width: 15rem;">
                            <div name="purchase_order_items[' . $query->id . '][unit_of_measurement]" class="input-group-append input-group-text border-0">' . $query->item->unitOfMeasurement->code . '</div>
                        </div>
                    ';
                } else {
                ...
                if ($query->purchaseOrder->status->name == 'Draft') {
                    return '
                        <div class="form-group">
                            <input id="price" name="purchase_order_items[' . $query->id . '][price]" type="text" class="input-count form-control text-end" placeholder="" value="' . number_format($query->price, 2, '.', '') . '" autocomplete="off">
                        </div>
                    ';
                } else {
            ...
            ->editColumn('total_price', function ($query) {
                if ($query->purchaseOrder->status->name == 'Draft') {
                    return '
                        <div class="form-group">
                            <label name="purchase_order_items[' . $query->id . '][total_price]">' . number_format($query->total_price, 0, '.', ',') . '</label>
                        </div>
                    ';
                } else {
            ...
            ->editColumn('note', function ($query) {
                if ($query->purchaseOrder->status->name == 'Draft') {
                    return '
                        <div class="form-group">
                            <textarea class="form-control" name="purchase_order_items[' . $query->id . '][note]" rows="1">' . $query->note . '</textarea>
                        </div>
                    ';
                } else {
                    return $query->note;
    ...
    {
        $query = new PurchaseOrderItems;
        return response()->json($query->store($request, $purchaseOrderId));
    }
}