@extends('layouts.app')
@section('title', 'Show ' . $query->vendor->name . ' Transaction on ' . date('F j, Y', strtotime($query->transaction_time)))
                    ...
                    <div class="col-sm-6 mb-4">
                        <div class="row">
                            <div class="col-sm-12 mb-4">
                                ...
                                <div>{!! $query->vendor ? $query->vendor->name : '<span class="text-muted fst-italic">Empty</span>' !!}</div>
                            </div>

                            <div class="col-sm-12 mb-4">
                                <label for="vendor-address" class="form-label">Vendor Address</label>

                                @if($query->vendorAddress)
                                <div>
                                    <div class="h4 w-100 mb-1">{{ $query->vendorAddress->name }}</div>
                                    <div>{!! $query->vendorAddress->phone ?? '<span class="text-muted fst-italic">-</span>' !!}</div>
                                    <div class="text-break">{!! $query->vendorAddress->full_address ?? '<span class="text-muted fst-italic">-</span>' !!}</div>
                                </div>
                                @else
...
<div class="row">
    <div class="col-lg-12 mb-3 mb-lg-0">
        <div class="card mb-3 mb-lg-5">
            <div class="card-header">
                <h4 class="card-header-title float-start">Purchase Order</h4>
                ...
                <div class="row">
                    <div class="col-sm-12">
                        <div class="datatable-custom table-responsive">
                            <table id="datatablePurchaseOrder"
                                ...
                                data-hs-datatables-options='{
                                    ...
                                    "processing": true,
                                    "serverSide": true,
                                    "ajax": "{{ route("purchase-orders.items.index", ["purchase_order" => $query->id]) }}",
                                    "columns": [
                                        {
                                            "data": "actions",
                                            "name": "actions",
                                ...
                                <tfoot class="thead-light">
                                    <tr>
                                        ...
                                        <th rowspan="1" colspan="1" class="text-end h1">Total Purchase</th>
                                        <th rowspan="1" colspan="1" class="text-end">
                                            <span name="purchase_order[total_purchase]" class="h1" <?= ($query->status->name == 'Draft' ? 'style="padding: 0rem 1rem;"' : '') ?>>{{ number_format($query->total_purchase, 0, '.', ',') }}</span>
                                        </th>
            ...
            <div class="row justify-content-center justify-content-sm-between">
                ...
                <div class="col-auto">
                    <div class="d-flex gap-3">
                        ...
                        @else
                        <a class="btn btn-ghost-light btn-discard" href="{{ route('purchase-orders.index') }}">Discard</a>
                        <div class="btn-group" role="group">
                            <span class="btn btn-white">
                                More
                            </span>
...
<script>
    (function () {
        ...
        const datatablePurchaseOrder = HSCore.components.HSDatatables.getItem('datatablePurchaseOrder');

        datatablePurchaseOrder.on('draw.dt', async function (e, settings, json, xhr) {
            const listTransaction = $("#datatablePurchaseOrder tbody").children();
        ...
        const handleGenerateItem = async (thisId) => {
            await HSCore.components.HSTomSelect.init(`select[name="purchase_order_items[${thisId}][item]"]`, {
                "valueField": 'id',
                "labelField": 'name',
                "searchField": ['name'],
            ...
            }, `purchase-order-item-${thisId}`);

            @if($query->status->name == 'Draft')
            purchaseOrderItem[thisId] = HSCore.components.HSTomSelect.getItem(`purchase-order-item-${thisId}`);
            @endif
        }
        ...
        const handleCalculating = async () => {
            ...
            await listTransaction.each(async function(index, transaction) {
                ...
                $(transaction).attr('data-quantity',    $(`[name="purchase_order_items[${thisId}][quantity]"]`).val()    ?? $(transaction).attr('data-quantity'));
                $(transaction).attr('data-price',       $(`[name="purchase_order_items[${thisId}][price]"]`).val()       ?? $(transaction).attr('data-price'));
                ...
                $(`[name="purchase_order_items[${thisId}][total_price]"]`).html(handleNumberFormat(totalPrice, 0));

                subTotal += totalPrice;
            });

            $(`[name="purchase_order[total_purchase]"]`).html(handleNumberFormat(subTotal, 0));
        };
        ...
        $(document).on('click', '.datatable-btn-status', async function (e) {
            ...
            const url           = `{{ route('purchase-orders.show', $query->id) }}/status`;
            ...
            await $.confirm({
                ...
                buttons: {
                    ...
                    okay: {
                        ...
                        btnClass: 'btn-secondary',
                        action: async function () {
                            history.back() ?? window.location.replace(`{{ route('purchase-orders.edit', $query->id) }}`);
                        }
                    },
                }
            });
        ...
        $(document).on('click', '.btn-save', async function (e) {
            const thisButton    = $(this);
            const url           = `{{ route('purchase-orders.items.index', $query->id) }}`
                                    ...
                                    await $.confirm({
                                        ...
                                        buttons: {
                                            edit: {
                                                ...
                                                action: function () {
                                                    window.location.replace(`{{ route('purchase-orders.edit', $query->id) }}`)
                                                }
                                            },
                                            close: {
                                                text: 'Still Show',
                                                ...
                                                keys: ['enter', 'esc'],
                                                action: function () {
                                                    datatablePurchaseOrder.ajax.reload(null, false);
                                                }
                                            },
                                        },
                                    });
        ...
        $(document).on('click', '.btn-items-create', async function (e) {   
            const listTransaction = $("#datatablePurchaseOrder tbody");
                ...
                >
                    ...
                    <td>
                        <div class="tom-select-custom">
                            <select name="purchase_order_items[${thisId}][item]" class="form-select form-item" autocomplete="off"
                    ...
                    <td>
                        <div class="input-group input-group-merge">
                            ...
                            <input name="purchase_order_items[${thisId}][quantity]" type="text" class="input-count form-control text-end" placeholder="" value="" autocomplete="off" style="min-width: 15rem;">
                            <div name="purchase_order_items[${thisId}][unit_of_measurement]" class="input-group-append input-group-text border-0"></div>
                        </div>
                    </td>
                    <td>
                        <div class="form-group">
                            <input id="price" name="purchase_order_items[${thisId}][price]" type="text" class="input-count form-control text-end" placeholder="" value="" autocomplete="off" style="min-width: 10rem;">
                        </div>
                    </td>
                    <td class="text-end">
                        <div class="form-group">
                            <span name="purchase_order_items[${thisId}][total_price]" style="padding: 0rem 1rem;">0</span>
                        </div>
                    </td>
                    <td class="text-end">
                        <div class="form-group">
                            <textarea class="form-control" name="purchase_order_items[${thisId}][note]" rows="1"></textarea>
                        </div>
                    </td>
                </tr>
            `);
        ...
        $(document).on('change', '.form-item', async function (e) {
            ...
            const thisValue = $(`select[name="purchase_order_items[${thisId}][item]"]`).val();

            let unitOfMeasurement = '';
            if (purchaseOrderItem[thisId].options[thisValue]?.unit_of_measurement) {
                unitOfMeasurement = purchaseOrderItem[thisId].options[thisValue].unit_of_measurement.code;
            } else if (purchaseOrderItem[thisId].options[thisValue]?.unit_of_measurement_code) {
                unitOfMeasurement = purchaseOrderItem[thisId].options[thisValue].unit_of_measurement_code;
            }

            $(`div[name="purchase_order_items[${thisId}][unit_of_measurement]"]`).html(unitOfMeasurement);
        });
