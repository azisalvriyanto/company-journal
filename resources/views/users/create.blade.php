            ...
            <div class="modal-body">
                <div class="row">
                    ...
                    <div class="col-sm-4 mb-4">
                        ...
                        <input id="address-email" name="address_email" type="email" class="form-control" placeholder="" value="" autocomplete="off">
                    </div>
                    <div class="col-sm-4 mb-4">
...
<script>
    (function () {
        ...
        $(document).on('click', '.btn-address-create', async function (e) {
            ...
            await $(`input[name="address_email"]`).val('');
        ...
        $(document).on('click', '.btn-address-edit', async function (e) {
            ...
            const email = $(`[name="${addressType}[${thisId}][email]"]`).html();
            ...
            await $(`input[name="address_email"]`).val(email);
        ...
        $(document).on('click', '#btn-address-save', async function (e) {
            ...
            const thisId = $(this).attr("data-id") != "" ? $(this).attr("data-id") : (addresses.children().length ? addresses.children().last().data('id')+1 : 1);
            ...
            const email = $(`input[name="address_email"]`);
            if (!email.val()) {
                errors.push(`<li class="list-pointer-item">Email not found</li>`);
            }
            ...
            if (errors.length > 0) {
                ...
                if ($(`#btn-address-save`).html() == 'Save') {
                    addresses.find(`[data-id="${thisId}"]`).html(`
                        <div class="row mb-2">
                            <div class="col-sm-12">
                                ...
                                <div name="${addressType}[${thisId}][email]">${ email.val() }</div>
                    ...
                    addresses.append(`
                        <div data-id="${thisId}" class="address mb-5" style="border-right: 10px solid ${ thisId == 1 ? `#377dff` : `transparent` };">
                            <div class="row mb-2">
                                <div class="col-sm-12">
                                <div name="${addressType}[${thisId}][name]" class="h3 mb-1">${ name.val() }</div>
                                <div name="${addressType}[${thisId}][email]" >${ email.val() }</div>
                                <div name="${addressType}[${thisId}][phone]" >${ phone.val() }</div>
                                <div name="${addressType}[${thisId}][full_address]" class="text-truncate">${ fullAddress.val() }</div>
                                    <div name="${addressType}[${thisId}][is_default]" hidden="">${ thisId == 1 ? `true` : `false` }</div>
                                </div>
                            </div>
                            <div class="row">
                                <div class="col-sm-12">
