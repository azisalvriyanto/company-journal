...
<li class="list-inline-item">
    <a class="list-separator-link" href="{{ route('invoices.index') }}">Invoices</a>
</li>
<li class="list-inline-item">
    <a class="list-separator-link" href="{{ route('invoices.show', $query->id) }}">
        {{ $query->customer->name }}
    ...
    <div class="col-lg-12 mb-3 mb-lg-0">
        <div class="card mb-3 mb-lg-5">
            <div class="card-header">
                <h4 class="float-start card-header-title">Invoice information</h4>
...
<div class="row">
    <div class="col-lg-12 mb-3 mb-lg-0">
        <div class="card mb-3 mb-lg-5">
            <div class="card-header">
                <h4 class="card-header-title float-start">Invoice</h4>
                        ...
                        <div class="datatable-custom table-responsive">
                            <table id="datatableInvoice"
                                ...
                                data-hs-datatables-options='{
                                    ...
                                    "processing": true,
                                    "serverSide": true,
                                    "ajax": "{{ route("invoices.items.index", ["invoice" => $query->id]) }}",
                                    "columns": [
                                        {
                                            "data": "actions",
                                            "name": "actions",
                                ...
                                <tfoot class="thead-light">
                                    <tr>
                                        ...
                                        <th rowspan="1" colspan="1" class="text-end">
                                            <span name="invoice[subtotal]" class="h4" <?= ($query->status->name == 'Draft' ? 'style="padding: 0rem 1rem;"' : '') ?>>{{ number_format($query->subtotal, 0, '.', ',') }}</span>
                                        </th>
                                    ...
                                    <tr>
                                        ...
                                        <th rowspan="1" colspan="1" class="text-end">
                                            @if($query->status->name == 'Draft')
                                            <input name="invoice[total_shipping]" type="text" class="input-count form-control text-end" placeholder="0" value="{{ number_format($query->total_shipping, 0, '.', '') }}" aria-label="0" autocomplete="off">
                                            @else
                                    ...
                                    <tr>
                                        ...
                                        <th rowspan="1" colspan="1" class="text-end">
                                            @if($query->status->name == 'Draft')
                                            <input name="invoice[total_discount]" type="text" class="input-count form-control text-end" placeholder="0" value="{{ number_format($query->total_discount, 0, '.', '') }}" aria-label="0" autocomplete="off">
                                            @else
                                        ...
                                        <th rowspan="1" colspan="1" class="pb-0 text-end">
                                            <div class="tom-select-custom tom-select-custom-end">
                                                <div id="taxSelect" class="input-group">
                                                    <input name="invoice[total_tax_value]" type="text" class="input-count form-control text-end" placeholder="0.00" value="{{ number_format($query->total_tax_value, 0, '.', '') }}" aria-label="0.00" autocomplete="off" style="min-width: 4rem;">
                                                    <select name="invoice[total_tax_type]" class="input-count js-select form-select" data-hs-tom-select-options='{
                                    ...
                                    <tr>
                                        ...
                                        <th rowspan="1" colspan="1" class="pt-0 text-end">
                                            <span name="invoice[total_tax]" style="padding: 0rem 1rem;">{{ number_format($query->total_tax, 0, '.', ',') }}</span>
                                        </th>
                                        <th rowspan="1" colspan="1"></th>
                                    </tr>
                                    @else
                                    ...
                                    <tr>
                                        <th rowspan="1" colspan="3"></th>
                                        <th rowspan="1" colspan="1" class="text-end h2">Total Invoice</th>
                                        <th rowspan="1" colspan="1" class="text-end">
                                            <span name="invoice[total_invoice]" class="h2" <?= ($query->status->name == 'Draft' ? 'style="padding: 0rem 1rem;"' : '') ?>>{{ number_format($query->total_invoice, 0, '.', ',') }}</span>
                                        </th>
                                        <th rowspan="1" colspan="1"></th>
                                    </tr>
                                    <tr hidden="">
                                        ...
                                        <th rowspan="1" colspan="1" class="text-end">
                                            <span name="invoice[total_amount_paid]" <?= ($query->status->name == 'Draft' ? 'style="padding: 0rem 1rem;"' : '') ?>>{{ number_format($query->total_amount_paid, 0, '.', ',') }}</span>
                                        </th>
                                        <th rowspan="1" colspan="1"></th>
                                    </tr>
                                    <tr hidden="">
                                        ...
                                        <th rowspan="1" colspan="1" class="text-end">
                                            <span name="invoice[total_due_balance]" class="h1" <?= ($query->status->name == 'Draft' ? 'style="padding: 0rem 1rem;"' : '') ?>>{{ number_format($query->total_due_balance, 0, '.', ',') }}</span>
                                        </th>
                                        <th rowspan="1" colspan="1"></th>
                                    </tr>
                                </tfoot>
            ...
            <div class="row justify-content-center justify-content-sm-between">
                ...
                <div class="col-auto">
                    <div class="d-flex gap-3">
                        ...
                        @else
                        <a class="btn btn-ghost-light btn-discard" href="{{ route('invoices.index') }}">Discard</a>
                        <div class="btn-group" role="group">
                            <span class="btn btn-white">
                                More
                            </span>
...
<script>
    (function () {
        ...
        const datatableInvoice = HSCore.components.HSDatatables.getItem('datatableInvoice');

        datatableInvoice.on('draw.dt', async function (e, settings, json, xhr) {
            const listTransaction = $("#datatableInvoice tbody").children();
        ...
        const handleGenerateItem = async (thisId, options=null, items=null) => {
            HSCore.components.HSTomSelect.init(`select[name="invoice_items[${thisId}][item]"]`, {
                "valueField": 'id',
                "labelField": 'name',
                "searchField": ['name'],
            ...
            @if($query->status->name == 'Draft')
            invoiceItem[thisId] = HSCore.components.HSTomSelect.getItem(`invoice-item-${thisId}`);
            @endif
        }
        ...
        const handleCalculating = async () => {
            ...
            await listTransaction.each(async function(index, transaction) {
                ...
                $(transaction).attr('data-quantity',    $(`[name="invoice_items[${thisId}][quantity]"]`).val()    ?? $(transaction).attr('data-quantity'));
                $(transaction).attr('data-price',       $(`[name="invoice_items[${thisId}][price]"]`).val()       ?? $(transaction).attr('data-price'));
                ...
                $(`[name="invoice_items[${thisId}][total_price]"]`).html(handleNumberFormat(totalPrice, 0));

                subTotal += totalPrice;
            });

            $(`[name="invoice[subtotal]"]`).html(handleNumberFormat(subTotal, 0));

            const totalShipping = handleSetNumber($(`[name="invoice[total_shipping]"]`).val());
            const totalDiscount = handleSetNumber($(`[name="invoice[total_discount]"]`).val());
            subTotal = subTotal + totalShipping - totalDiscount;

            const totalTaxValue = handleSetNumber($(`[name="invoice[total_tax_value]"]`).val());
            const totalTaxType = $(`[name="invoice[total_tax_type]"]`).val();
            const totalTax = handleSetNumber(totalTaxType == 'Percent' ? subTotal*(totalTaxValue/100) : totalTaxValue);
            if (totalTaxType == 'Percent') {
                $(`[name="invoice[total_tax]"]`).html(handleNumberFormat(totalTax, 0));
            } else {
                $(`[name="invoice[total_tax]"]`).html('');
            }

            const totalInvoice = subTotal + totalTax;
            $(`[name="invoice[total_invoice]"]`).html(handleNumberFormat(totalInvoice, 0));

            const totalAmountPaid = handleSetNumber($(`[name="invoice[total_amount_paid]"]`).html());
            $(`[name="invoice[total_due_balance]"]`).html(handleNumberFormat(totalInvoice - totalAmountPaid, 0));
        };
                ...
                buttons: {
                    ...
                    okay: {
                        ...
                        action: async function () {
                            history.back() ?? window.location.replace(`{{ route('invoices.index') }}`);
                        }
                    },
                }
            });
        ...
        $(document).on('click', '.datatable-btn-status', async function (e) {
            ...
            const url           = `{{ route('invoices.show', $query->id) }}/status`;
            ...
            await $.confirm({
                ...
                buttons: {
                    ...
                    okay: {
                        ...
                        btnClass: 'btn-secondary',
                        action: async function () {
                            history.back() ?? window.location.replace(`{{ route('invoices.edit', $query->id) }}`);
                        }
                    },
                }
            });
        ...
        $(document).on('click', '.btn-save', async function (e) {
            const thisButton    = $(this);
            const url           = `{{ route('invoices.items.index', ['invoice' => $query->id]) }}`;
                                    ...
                                    await $.confirm({
                                        ...
                                        buttons: {
                                            show: {
                                                ...
                                                btnClass: 'btn-primary',
                                                action: function () {
                                                    window.location.replace(`{{ route('invoices.show', $query->id) }}`)
                                                }
                                            },
                                            close: {
                                                text: 'Still Edit',
        ...
        $(document).on('click', '.btn-destroy', async function (e) {
            const url = `{{ route('invoices.show', $query->id) }}`
            await $.confirm({
                title: 'Confirmation!',
                                    ...
                                    $.confirm({
                                        ...
                                        buttons: {
                                            close: {
                                                ...
                                                keys: ['enter', 'esc'],
                                                action: function () {
                                                    window.location.replace(`{{ route('invoices.index') }}`);
                                                }
                                            },
                                        }
                                    });
        ...
        $(document).on('click', '.btn-items-create', async function (e) {   
            const listTransaction = $("#datatableInvoice tbody");
                ...
                >
                    ...
                    <td>
                        <div class="tom-select-custom">
                            <select name="invoice_items[${thisId}][item]" class="form-select form-item" autocomplete="off"
                    ...
                    <td>
                        <div class="input-group input-group-merge">
                            <input name="invoice_items[${thisId}][quantity]" type="text" class="input-count form-control text-end" placeholder="" value="" autocomplete="off" style="min-width: 10rem;">
                            <div name="invoice_items[${thisId}][unit_of_measurement]" class="input-group-append input-group-text"></div>
                        </div>
                    </td>
                    <td>
                        <div class="form-group">
                            <input id="price" name="invoice_items[${thisId}][price]" type="text" class="input-count form-control text-end" placeholder="" value="" autocomplete="off" style="min-width: 10rem;">
                        </div>
                    </td>
                    <td class="text-end">
                        <div class="form-group">
                            <span name="invoice_items[${thisId}][total_price]" style="padding: 0rem 1rem;">0</span>
                        </div>
                    </td>
                    <td class="text-end">
                        <div class="form-group">
                            <textarea class="form-control" name="invoice_items[${thisId}][note]" rows="1"></textarea>
                        </div>
                    </td>
                </tr>
            `);
        ...
        $(document).on('change', '.form-item', async function (e) {
            ...
            const thisValue = $(`select[name="invoice_items[${thisId}][item]"]`).val();

            console.log(invoiceItem[thisId].options[thisValue])
            let unitOfMeasurement = '';
            if (invoiceItem[thisId].options[thisValue]?.unit_of_measurement) {
                unitOfMeasurement = invoiceItem[thisId].options[thisValue].unit_of_measurement.code;
            } else if (invoiceItem[thisId].options[thisValue]?.unit_of_measurement_code) {
                unitOfMeasurement = invoiceItem[thisId].options[thisValue].unit_of_measurement_code;
            }

            $(`div[name="invoice_items[${thisId}][unit_of_measurement]"]`).html(unitOfMeasurement);
        });
