@extends('layouts.app')
@section('title', 'Edit ' . $query->customer->name . ' Transaction on ' . date('F j, Y', strtotime($query->transaction_time)))
...
<li class="list-inline-item">
    <a class="list-separator-link" href="{{ route('invoices.edit', $query->id) }}">Edit</a>
</li>
@endsection
                            ...
                            <div class="col-sm-12 mb-4">
                                ...
                                <div class="tom-select-custom">
                                    <select id="customer" name="customer" class="form-select" autocomplete="off"
                                        ...
                                        <option
                                            selected=""
                                            value="{{ $query->customer->id }}"
                                            data-id="{{ $query->customer->id }}"
                                            data-name="{{ $query->customer->name }}"
                                        ></option>
                                    </select>
                                </div>
                            </div>
...
<script>
    (function () {
        ...
        const handleGeneratingCustomerAddress = async (customerAddresses, customerAddressId=null) => {
            HSCore.components.HSTomSelect.init(`select[name="customer_address"]`, {
                "valueField": 'id',
                "labelField": 'full_address',
                "searchField": ['full_address'],
                "options": customerAddresses,
                "items": customerAddressId ? [customerAddressId] : [],
                "render": {
                    option: function(data, escape) {
        ...
        $(document).on('change', '#customer', async function (e) {
            ...
            if (!customerAddress) {
                handleGeneratingCustomerAddress({!! json_encode($query->customer->billingAddresses) !!}, '<?= $query->customerAddress->id ?>');
            } else {
            ...
            await $.confirm({
                ...
                buttons: {
                    ...
                    okay: {
                        ...
                        btnClass: 'btn-secondary',
                        action: async function () {
                            history.back() ?? window.location.replace(`{{ route('invoices.show', $query->id) }}`);
                        }
                    },
                }
            });
